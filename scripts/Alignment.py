...
def optimize(imga, imgb):
    ...
    for t in range(2):
        ...
        for r in range(36):
            imgx = rotate(imgbt, r*10)
            imgx = (imgx > 0.5).astype(np.uint8)
            imgx = skm.remove_small_objects(imgx, min_size=1000, connectivity=1, in_place=False)
            imgx = skm.remove_small_holes(imgx, area_threshold=1000, connectivity=1, in_place=False)
            istart = 0
            jstart = 0
