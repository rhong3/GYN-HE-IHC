    ...
    if opt.mode in ['X1', 'X2', 'X3', 'X4', 'F1', 'F2', 'F3', 'F4']:
        ...
        try:
            ...
            if opt.pdmd == 'histology':
                ...
                for idx, row in ref.iterrows():
                    big_images.append(
                        [row['name'], int(row['subtype_{}'.format(opt.pdmd)]), img_dir + "{}/".format(str(row['name'])),
                         row['age'], row['BMI']])
            elif opt.pdmd in ['MSI_PMS2', 'MSI_MSH6', 'MSI_MSH2', 'MSI_MLH1', 'MSI_PMS2-MLH1', 'MSI_MSH2-MSH6',
                              'MSI_PMS2-MLH1-MSH2-MSH6', 'Serous-like_P53']:
                ref = ref.loc[ref['subtype_0NA'] == 0]
                for idx, row in ref.iterrows():
                    big_images.append(
                ...
                ref = ref.dropna(subset=[opt.pdmd.split('_')[0]])
                for idx, row in ref.iterrows():
                    big_images.append(
